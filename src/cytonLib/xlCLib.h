...
//		std::string cwd=XLLib::dirCwd();
//		std::string cmd=XLLib::stringFormat("ln -s %s/%s %s/%s",
//				cwd.c_str(), src.c_str(),
//				cwd.c_str(), des.c_str());
	...
	{
		//		std::string cmd=XLLib::stringFormat("mv %s %s", src.c_str(), des.c_str());
		//		system(cmd.c_str());
	...
	static std::string endTime(const XLLibTime& startTime, double* pTime=NULL)
	{
		...
		if(diff>1e-2*tMax){
			os<<stringFormat("%.1e/%.1es", timeUser, time);
		}else{
			os<<stringFormat("%.1es", time);
		}
